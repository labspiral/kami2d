// demo_mfcDlg.cpp : 구현 파일
//

#include "stdafx.h"
#include "demo_mfc.h"
#include "demo_mfcDlg.h"
#include "..\demo\testview.h"
...
Cdemo_mfcDlg::Cdemo_mfcDlg(CWnd* pParent /*=NULL*/)
	: CDialog(Cdemo_mfcDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(Cdemo_mfcDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
...
BOOL Cdemo_mfcDlg::OnInitDialog()
{
	...
	_pView = new TestView;
	_pView->initialize( m_hWnd);

	RECT rect;
	GetClientRect(&rect);	
	_pView->onResize( rect.right, rect.bottom);
...
void Cdemo_mfcDlg::OnSize(UINT nType, int cx, int cy)
{
	CDialog::OnSize(nType, cx, cy);

	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	if (_pView)
		_pView->onResize(cx, cy);
}
